...
def format_time(secs):
    milliseconds = math.floor(int(secs * 1000 % 1000)/1000)
    seconds = int(round(secs % 60, 1))
    minutes = int(round(secs // 60, 1))

    return f"{minutes:02d}:{seconds:02d}.{milliseconds:02d}"

def draw_top_bar(win, elapsed_time, target_pressed, misses):
    pygame.draw.rect(win, "grey", (0, 0, WIDTH, TOP_BAR_HEIGHT))
    time_label = LABEL_FONT.render(f"Time: {elapsed_time:.2f}", 1, "white")
    speed = round(target_pressed / elapsed_time, 1)
    speed_label = LABEL_FONT.render(f"Speed: {speed} t/s", 1, "white")
    hits_label = LABEL_FONT.render(f"Hits: {target_pressed}", 1, "white")
    lives_label = LABEL_FONT.render(f"Lives Left: {LIVES - misses}", 1, "white")
